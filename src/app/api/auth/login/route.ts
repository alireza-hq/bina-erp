...
export async function POST(request: Request) {
  ...
  try {
    ...
    const user = localUser?.passwordHash
      ? verifyPassword(parsed.data.password, localUser.passwordHash)
        ? localUser
        : null
      : await authenticateDirectoryUser(parsed.data.username, parsed.data.password).then(
          syncDirectoryUser,
        );
